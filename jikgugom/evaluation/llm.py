"""LLM 호출 추상화 — real(Gemini)/mock 자동 전환.

[원칙] LLM 호출은 반드시 이 파일을 거친다(다른 곳에서 LLM SDK/REST 직접 호출 금지).
[중요] GEMINI_API_KEY 가 없으면 자동으로 mock 모드 → 키 없이도 전체 파이프라인이 돈다.
...
class LLM:
    ...
    def __init__(self, model: str | None = None) -> None:
        self._model = model   # None이면 Gemini 기본 모델(GEMINI_MODEL/flash)
    ...
    def score_market_fit(
        ...
        try:
            raw = self._call_llm(product, signals)
    ...
    # ── real 모드 내부 (Gemini) ──────────────────────────────
    def _call_llm(self, product: SourceProduct, signals: MarketSignals) -> str:
        system = (
        ...
        return generate_text(system, user, model=self._model,
                             max_tokens=256, json_output=True)
